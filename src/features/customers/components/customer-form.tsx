...
export function CustomerForm({ customer }: CustomerFormProps) {
  ...
  function finish(successMessage: string, customerId: string) {
  ...
  async function onSubmit(values: CustomerFormValues) {
    if (customer) {
      ...
      if (result.ok) finish("customers.toasts.updated", customer.id);
      else showFailure(result);
    } else {
      const result = await createCustomer(values);
      if (result.ok) finish("customers.toasts.created", result.id);
      else showFailure(result);
    }
  }
        ...
        {isCompany && (
          <>
            ...
            <Field data-invalid={!!errors.contactPerson}>
              <FieldLabel htmlFor="contactPerson">{t("customers.fields.contactPerson")}</FieldLabel>
              <Input
                id="contactPerson"
                autoComplete="off"
